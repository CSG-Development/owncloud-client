    ...
    for (const auto &item : std::as_const(items)) {
        ...
        if (item->isDirectory()) {
            ...
            if (item->instruction() == CSYNC_INSTRUCTION_REMOVE) {
                ...
                for (auto &dir_item : directories) {
                    if (dir_item.second->item()->instruction() == CSYNC_INSTRUCTION_UPDATE_METADATA) {
                        dir_item.second->item()->setInstruction(CSYNC_INSTRUCTION_NONE);
                        _anotherSyncNeeded = true;
                    }
                }
            } else {
