...
void NetworkSettings::loadProxySettings()
{
    ...
    ProxyCredentials::load(this, [this, proxyGeneration](const QString &password) {
        if (proxyGeneration != ClientProxy::proxyGeneration()) {
            return;
        }
        _ui->passwordLineEdit->setText(password);
        ClientProxy::applyProxy(password);
    });
}
...
void NetworkSettings::saveProxySettings()
{
    ...
    const bool storeProxyPassword = _ui->manualProxyRadioButton->isChecked() && !_ui->hostLineEdit->text().isEmpty()
                                    && _ui->authRequiredcheckBox->isChecked() && !_ui->passwordLineEdit->text().isEmpty();
    if (storeProxyPassword) {
        ProxyCredentials::store(_ui->passwordLineEdit->text());
    }
    else {
        ProxyCredentials::remove();
    }

    ClientProxy::bumpProxyGeneration();
    ClientProxy::applyProxyAndRefresh(_ui->passwordLineEdit->text());
}
