...
function(__addUrlIcons QRC_PATH)
    file(GLOB_RECURSE CURATOR_URL_ICONS "${OEM_THEME_DIR}/theme/universal/urlIcons/*")
    foreach(icon ${CURATOR_URL_ICONS})
        get_filename_component(iconName ${icon} NAME_WE)
        __addIcon(${QRC_PATH} "universal" "urlIcons/${iconName}")
    endforeach()
endfunction()

function(generate_theme TARGET CURATOR_SIDEBAR_ICONS_OUT)
    if(NOT "${OEM_THEME_DIR}" STREQUAL "${PROJECT_SOURCE_DIR}/src/resources/")
        ...
        file(GLOB_RECURSE CURATOR_SIDEBAR_ICONS "${OEM_THEME_DIR}/theme/colored/*-${APPLICATION_ICON_NAME}-sidebar.png")
    else()
    ...
    set(${CURATOR_SIDEBAR_ICONS_OUT} ${CURATOR_SIDEBAR_ICONS} PARENT_SCOPE)
endfunction()


function(generate_legacy_icons theme_dir OUT)
    # allow legacy file names
    file(GLOB_RECURSE CURATOR_ICONS_OLD "${theme_dir}/colored/${APPLICATION_ICON_NAME}-icon-*.png")
    foreach(icon ${CURATOR_ICONS_OLD})
        ...
        list(APPEND CURATOR_ICONS ${out_name})
    endforeach()
    set(${OUT} ${CURATOR_ICONS} PARENT_SCOPE)
endfunction()
