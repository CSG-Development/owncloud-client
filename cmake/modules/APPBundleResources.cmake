...
function(__addUrlIcons QRC_PATH)
    file(GLOB_RECURSE APPLICATION_URL_ICONS "${OEM_THEME_DIR}/theme/universal/urlIcons/*")
    foreach(icon ${APPLICATION_URL_ICONS})
        get_filename_component(iconName ${icon} NAME_WE)
        __addIcon(${QRC_PATH} "universal" "urlIcons/${iconName}")
    endforeach()
endfunction()

function(app_generate_theme TARGET APPLICATION_SIDEBAR_ICONS_OUT)
    if(NOT "${OEM_THEME_DIR}" STREQUAL "${PROJECT_SOURCE_DIR}/src/resources/")
        ...
        file(GLOB_RECURSE APPLICATION_SIDEBAR_ICONS "${OEM_THEME_DIR}/theme/colored/*-${APPLICATION_ICON_NAME}-sidebar.png")
    else()
        file(GLOB_RECURSE APPLICATION_SIDEBAR_ICONS "${OEM_THEME_DIR}/theme/colored/*-${APPLICATION_ICON_NAME}-icon-sidebar.png")
    endif()
...
function(app_generate_legacy_icons theme_dir OUT)
    # allow legacy file names
    file(GLOB_RECURSE APPLICATION_ICONS_OLD "${theme_dir}/colored/${APPLICATION_ICON_NAME}-icon-*.png")
    foreach(icon ${APPLICATION_ICONS_OLD})
        ...
        list(APPEND APPLICATION_ICONS ${out_name})
    endforeach()
    set(${OUT} ${APPLICATION_ICONS} PARENT_SCOPE)
endfunction()
...
function(app_add_resources_to_target)
    set(options "")
